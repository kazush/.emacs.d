...
(use-package-if-corfu
 ...
 :config
 (defun start-corfu-terminal-mode ()
   (unless (display-graphic-p)
     (corfu-terminal-mode +1)))

 (add-hook 'server-after-make-frame-hook #'start-corfu-terminal-mode)
 (start-corfu-terminal-mode)
 )
#+end_src
...
(use-package corfu-candidate-overlay
  ...
  (defun my/corfu-candidate-overlay-mode (&optional win-or-frame)
    (let* ((b (current-buffer))
      ...
      ;; (message "buf:%s, win:%s, livep:%s" b w (window-live-p w))
      (when (and
             (window-live-p w)
             (not (or
  ...
  (add-to-list 'window-buffer-change-functions 'my/corfu-candidate-overlay-mode)
