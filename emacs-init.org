...
(defun my/llm-set-api-key (provider-list)
  "Set the environment variable with the API key for each specified provider.
   PROVIDER-LIST can be a symbol or a list of symbols.
   'ALL represents all registered providers."
  (let ((provider-conf-alist (if (null providers) '() my/llm-provider-alist))
        (provider-list (if (eq providers t) nil
                         (if (listp providers) providers (list providers)))))
    (dolist (p provider-conf-alist)
      (let ((name (car p))
