...
(defun my/get-display-buffer-action-for-shellish (shellish-mode use-tab-p)
  ...
  (let ((cur-buf (current-buffer))
    ...
    (cond
     ...
     ;; ((my/shellish-buffer-p (current-buffer) shellish-mode)
     ;;  `(display-buffer-same-window))
     (use-tab-p
...
(defun my/shellish-display-buffer (buf &optional omni-display)
  "Selectively switch to an existing window or tab displaying BUF-NAME."
  (let* ((tab (tab-bar-get-buffer-tab buf)))
...
(defun my/shellish-toggle-buffer (arg shellish-mode &optional buf-index)
  ...
  (let* ((use-tab-flipped (= arg 4))
         ...
         (tab-exists-p (member tab-name
                               (mapcar (lambda (e) (alist-get 'name e)) (tab-bar-tabs))))
         (display-buffer-overriding-action (my/get-display-buffer-action-for-shellish
                                            shellish-mode new-use-tab-p)))
    (cond
     ...
     ((eq b (current-buffer))
      (if use-tab-p
          ;; if current-tab is dedicated tab, close it.
          (tab-close)
        ;; if current buffer is shellish buffer, close it.
        (condition-case err
            (delete-window (get-buffer-window (current-buffer)))
          (error
           (switch-to-prev-buffer)))))
     (b ;; switch to existing shellish buffer
      (my/shellish-display-buffer b
                                  (my/shellish-get-omni-display-for-buffer (buffer-name b)))))
