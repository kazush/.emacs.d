...
(use-package shr
  ...
  (defun my/shr-browse-url (&optional external mouse-event new-window)
...
(use-package consult
  ...
  :bind (;; C-c bindings (mode-specific-map)
         ("C-c r" . consult-recent-file-other-window)
  ...
  (consult-customize
   ...
   consult-recent-file consult-recent-file-other-window
...
simple macros are defined using built-in Tempo.

#+begin_src emacs-lisp
(use-package tempo
  :config
  (add-to-list 'completion-at-point-functions #'tempo-complete-tag)
  ;; keys are in the form of either '@<word>' or '<<word>'.
  (setq my/tempo-match-finder "\\W\\([@<][[:word:]]+\\)\\=")

  ;; Template definitions.
  (tempo-define-template "time" '((format-time-string "%H:%M:%S") p >) "@time" "Insert current time")
  (tempo-define-template "today" '((format-time-string "%Y-%m-%d") p >) "@today" "Insert today's date")
  (tempo-define-template "tomorrow" '((format-time-string "%Y-%m-%d" (time-add (current-time) (* 24 3600))) p >) "@tomorrow" "Insert tomorrow's date")
  (tempo-define-template "yesterday" '((format-time-string "%Y-%m-%d" (time-subtract (current-time) (* 24 3600))) p >) "@yesterday" "Insert yesterday's date")

  (defun my/tempo-setup ()
    "Setup my Tempo configuration for the current buffer."
    (interactive)
    (setq-local tempo-match-finder my/tempo-match-finder)
    )
  (add-hook 'org-mode-hook 'my/tempo-setup 100)
  (add-hook 'text-mode-hook 'my/tempo-setup 100)
  )
#+end_src
...
TBH I don't know if this is still needed.
...
(use-package whitespace
  ...
  :config
  (set-face-background 'whitespace-tab (if window-system "gray25" "color-52"))
...
(use-package go-translate
  ...
  (setq gt-default-translator
        (gt-translator :taker (gt-taker :langs '(en ja) :text 'word)
...
(use-package consult-omni
  ...
  :bind (("M-s /" . consult-omni-multi)
         ("C-c / a" . consult-omni-apps)
         ("C-c / e" . consult-omni-elfeed)
         ("C-c / g" . consult-omni-google)
         ("C-c / G" . consult-omni-gh)
         ("C-c / l" . consult-omni-locate)
         ("C-c / s" . consult-omni-multi-static)
         ("C-c / w" . consult-omni-wikipedia)
         ("C-c / y" . consult-omni-invidious))
  :custom
