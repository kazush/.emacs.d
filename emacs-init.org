...
(defun my/create-custom-fontsets  ()
  "Create custom fontsets for mixed fonts."
  (create-fontset-from-ascii-font "Noto Serif:weight=regular" nil "latex")
  (set-fontset-font "fontset-latex" 'unicode
                    (font-spec :family "Harano Aji Mincho"
                               :weight 'regular))
  (create-fontset-from-ascii-font "Noto Serif:weight=regular" nil "serif")
  (set-fontset-font "fontset-serif" 'unicode
                    (font-spec :family "Noto Serif CJK JP"
                               :weight 'regular))
  (create-fontset-from-ascii-font "Noto Sans:weight=regular" nil "sans")
  (set-fontset-font "fontset-sans" 'unicode
...
(defun my/setup-fonts ()
  ...
  (custom-theme-set-faces
   ...
   `(fixed-pitch ((t (:family ,my/default-font-family)))))
  )
#+end_src
...
#+begin_src emacs-lisp
(use-package tab-bar
  :bind (("C-z w" . tab-bar-move-window-to-tab)))
#+end_src
...
return the same window."
  (let ((win (get-buffer-window buf)))
    (cond
     (win win)
     ((or (< (count-windows) my/dba-min-windows))
...
(use-package org
  ...
  (org-todo-keywords
   '((sequence "TODO(t)" "IN-PROGRESS(i)" "WAIT(w)" "|" "DONE(d)" "CANCELLED(c)" "SOMEDAY(s)")))
  (org-log-done 'time)
  (org-capture-templates nil) ;; Migrated to org-roam-capture.
  (org-agenda-files nil)  ;; will be configured in org-roam package config.
  (org-tag-alist
   '(("ai")
     ("algo")
     ("blog")
...
(use-package org-roam
  ...
  :bind (("C-c n f" . org-roam-node-find)
         ...
         ("C-c o d" . my/org-roam-capture-diary)
         ("C-c o p" . my/org-roam-capture-project)
         :map org-mode-map
  ...
  (org-roam-capture-templates
   '(("d" "default" plain "%?" :target
     ...
     ("c" "contact" entry "* ${title}\n%?" :target
      (file+head "contacts.org.gpg" "#+title: Personal Contacts\n")
      :unnarrowed t)
     ("e" "encrypted" plain "%?" :target
      (file+head "secure/${slug}.org.gpg" "#+title: ${title}\n")
      :unnarrowed t)))
  :init
  ;; Add diary and project files to org-agenda.
  (setq org-agenda-files
        (append org-agenda-files '("~/org/roam/diary/"
                                   "~/org/roam/projects/")))
  ...
  (defun my/org-roam-create-new-project-file (prj file)
    "Create a new file node at FILE with property project=PRJ."
    (let* ((org-roam-capture-templates
           ...
           (node (org-roam-node-create
                  :file file :title (concat "Project: " prj)
                  :properties `(("project" .  ,prj))))
           (hook (lambda () (org-entry-put (point) "project" prj))))
      (add-hook 'org-roam-capture-new-node-hook hook)
      (org-roam-capture- :node node)
      (remove-hook 'org-roam-capture-new-node-hook hook)))

  (defun my/org-roam-capture-project (prj &optional key)
    "Capture journal or task for a selected project."
    (interactive
     (list
      (completing-read
       "Project: "
       (mapcar (lambda (str) (string-replace ".org" "" (file-name-base str)))
               (file-expand-wildcards
                (format "%s/projects/*.org" org-roam-directory)))
       nil 'confirm)))
    (let* ((prj-file (format "%s/projects/%s.org" org-roam-directory prj))
           (org-capture-templates
            `(("j" "add a new journal entry to the selected project"
               entry (file+olp+datetree ,prj-file "Journal")
               "* %?")
              ("t" "add a new task entry to the selected project"
               entry (file+olp ,prj-file "Tasks")
               "* TODO %?\n"))))
      (unless (file-exists-p prj-file)
        (my/org-roam-create-new-project-file prj prj-file))
      (org-capture nil key)))

  ;; Helper functions for diary

  (defun my/org-roam-create-new-diary-file (file)
    "Create a new file node at FILE."
    (let* ((org-roam-capture-templates
            `(("d" "default - create a new node"
               plain "\n* Tasks\n\n* Diary\n%?"
               :target (file+head ,file "#+title: ${title}\n")
               :immediate-finish t)))
           (node (org-roam-node-create
                  :file file :title (format-time-string "Diary %Y"))))
      (org-roam-capture- :node node)))

  (defun my/org-roam-capture-diary (&optional key)
    "Capture diary."
    (interactive)
    (let* ((file (format "%s/diary/%s"
                         org-roam-directory
                         (format-time-string "diary-%Y.org")))
           (org-capture-templates
            `(("d" "add a new diary entry"
               entry (file+olp+datetree ,file "Diary")
               "* %?")
              ("t" "add a new task entry"
               entry (file+olp ,file "Tasks")
               "* TODO %?\n"))))
      (unless (file-exists-p file)
        (my/org-roam-create-new-diary-file file))
      (org-capture nil key)))
    ...
    (let* ((authinfo (car (auth-source-search :host "org-gcal")))
      ...
      (setq org-gcal-fetch-file-alist
            `((,calendar . "~/org/gcal.org")
              ...
              ("monex.co.jp_vm8hs3tiu5cp7gdlk04bla79o0@group.calendar.google.com" . "~/org/economic-calendar.org")))
...
(use-package ellama
  ...
  (setopt ellama-providers
          '(("gemini-flash" . (my/make-llm-gemini
            ...
            ("gemini-exp" . (my/make-llm-gemini
                             :chat-model "gemini-exp-1206"))
