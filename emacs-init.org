  ...
  (let ((win (get-buffer-window buf))
    ...
    (if win win
      (if (and (= (count-windows) 1)
        ...
        (when (< (count-windows) my/dba-min-windows)
          (setq win (split-window-horizontally))))
      (when win
...
(use-package org-roam
  ...
  (setq org-agenda-files
        (append org-agenda-files
                '("~/org/roam" "~/org/roam/secure"
                  "~/org/roam/daily" "~/org/roam/daily/secure")))
  :config
...
\\fi"
  ...
  ;; Adjust left margin on overlay equations.
  ;; source: https://www.reddit.com/r/emacs/comments/15vt0du/centering_latex_previews_in_org97/
  (defun my/ov-at (&optional point)
  ...
  (defun my/org-justify-fragment-overlay (beg end image &optional imagetype)
    "Only equations at the beginning and also end of a line are justified."
    (when (and (= beg (line-beginning-position)) (= end (line-end-position)))
      (let* ((ov (my/ov-at))
             (disp (overlay-get ov 'display)))
        ;; (overlay-put ov 'line-prefix `(space :align-to (- center (0.5 . ,disp))))
        (overlay-put ov 'line-prefix `(space :relative-width 10)))
      ))
  (advice-add 'org--make-preview-overlay :after 'my/org-justify-fragment-overlay)
