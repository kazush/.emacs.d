...
(use-package elfeed
  ...
  ;; For headless update with emacsclient --eval.
  (defun my/elfeed-update-headless ()
    "Update elfeed feeds in a headless way."
    (interactive)
    (require 'elfeed)
    (require 'elfeed-org)
    (unless (member #'elfeed-org-run-new-entry-hook elfeed-new-entry-hook)
      (elfeed-org))
    (elfeed-db-load)
    (rmh-elfeed-org-process-advice)
    (message "[%s] elfeed feeds updated %s" (format-time-string "%Y-%m-%d %H:%M:%S")
             (if (eq (elfeed-update) :success)
                 "successfully" "with an error")))
