...
(use-package elfeed
  ...
  :commands (my/elfeed-update-headless)
  :config
...
triggered.
  ...
  ;; Load index from filesystem before launching elfeed to try to avoid using stale index.
  ;; This happens when elfeed db is synced among hosts using syncthing.
  (defun my/elfeed ()
    "Launch elfeed after reloading index from filesystem."
    (interactive)
    (elfeed-db-load)
    (elfeed)))
