...
(defun my/set-fontset-font (&optional font size fset)
  ...
  (interactive "sFont: \nnSize: \ni")
  (let ((newfont (my/make-font-str (or font my/defaultfont)
...
(defun my/set-fontset-emoji-font (&optional font size fset)
  ...
  (let ((emoji-font (my/make-font-str (or font my/default-emoji-font)
                                      (or size my/default-emoji-font-size)))
        (fontset (or fset (face-attribute 'default :fontset))))
...
(defun my/set-fontset-jp-font (&optional font size fset)
  ...
  (let ((font-xlfd (my/make-font-str (or font my/default-jp-font)
                                     (or size my/default-jp-font-size)))
        (fontset (or fset (face-attribute 'default :fontset))))
...
(use-package org
  ...
  (defun my/org-get-entry-date-str ()
    "Get formatted date string from current org entry.
If TIMESTAMP exists, returns its formatted date.
Otherwise returns scheduled and deadline dates connected with '~'.
Format: '<scheduled>~<deadline>', '<scheduled>', or '~<deadline>'"
            ...
            (lambda (date-str)
              (when date-str
                (let* ((time (org-time-string-to-time date-str))
                       ...
                       (format-str (if (and todo (not (equal org-agenda-type 'agenda)))
                                       (if has-time "%Y-%m-%d %H:%M" "%Y-%m-%d")
                                     (if has-time "%H:%M" ""))))
                  (format-time-string format-str time)))))
           (format-date-relative
            (lambda (date-str &optional empty-time start-date)
              (when date-str
                (let* ((time (org-time-string-to-time date-str))
                       (date-ymd (format-time-string "%Y-%m-%d" time))
                       (start-ymd (if start-date
                                      (format-time-string "%Y-%m-%d" start-date)
                                    (format "%04d-%02d-%02d"
                                            (nth 2 org-agenda-current-date)
                                            (nth 0 org-agenda-current-date)
                                            (nth 1 org-agenda-current-date))))
                       (has-time (nth 1 (org-parse-time-string date-str t)))
                       (format-str (if (string= date-ymd start-ymd)
                                       (if has-time "%H:%M" (if empty-time "" "23:59"))
                                     (message "%s <> %s" date-ymd start-ymd)
                                     (if has-time "%Y-%m-%d %H:%M" "%Y-%m-%d"))))
                  (format-time-string format-str time))))))
      (cond
       ;; Return timestamp if exists
       (timestamp
        (funcall format-date timestamp))
       ;; Return combined scheduled and deadline if both exist
       ((and scheduled deadline)
        (format "%s〜%s"
                (funcall format-date-relative scheduled)
                (funcall format-date-relative deadline nil
                         (org-time-string-to-time scheduled))))
       ;; Return only scheduled if it exists
       (scheduled
        (funcall format-date-relative scheduled t))
       ;; Return deadline with ~ prefix if only deadline exists
       (deadline
        (let ((date-str (funcall format-date-relative deadline t)))
          (if (string= date-str "") date-str (format "〜%s" date-str))))
