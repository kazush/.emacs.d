...
(use-package consult
  ...
  (defun my/consult-line (&optional prefix)
    (interactive "P")
    (if prefix
        (consult-line)
      (if (use-region-p)
...
(use-package whitespace
  ...
  :config
  (set-face-background 'whitespace-tab (if window-system "gray70" "color-52"))
...
(add-to-list 'compilation-error-regexp-alist '("^\\(.+\\)(\\([0-9]+\\),\\([0-9]+\\)) : \\(error\\|warning\\).*$" 1 2 3))
#+end_src
