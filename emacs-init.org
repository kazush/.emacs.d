...
(use-package recentf
  ...
  (setq recentf-exclude
   (append recentf-exclude '("/org/roam/"
                             "/.emacs.d/.cache/"
                             "/.emacs.d/bookmarks")))
  (recentf-mode))
#+end_src
...
(defun my/switch-to-scratch-tab ()
  (interactive)
  (my/switch-to-buffer-other-tab (get-scratch-buffer-create)))
(define-key tab-prefix-map "s" #'my/switch-to-scratch-tab)

;; Use C-h as DEL/BS and C-] as C-h (help-command).
;; key-trsnslate is now used to make C-h (translated to DEl/BS) work with ghostel.
;; (define-key key-translation-map (kbd "C-h") (kbd "DEL"))
;; (define-key key-translation-map (kbd "C-]") (kbd "C-h"))
(defun my/setup-keymap ()
...
(use-package mozc-modeless
  :straight (:type git :host github :repo "kiyoka/mozc-modeless-emacs")
  :after mozc
  :config
...
(use-package tab-bar
  :bind (("C-x C-f" . my/find-file)
         ("C-x k" . my/tab-bar-kill-buffer-and-window-or-tab)
         ("C-x K" . kill-buffer)
         ("C-x b" . my/switch-to-buffer)
         :map tab-prefix-map
         ("C-b" . tab-bar-switch-to-tab)
         ("C-n" . tab-next)
         ("C-p" . tab-previous)
  ...
  (defun my/switch-to-buffer-other-tab (&optional buf)
    "Switch to an existing tab if the buffer already exists or to a new tab."
    (interactive)
    (let* ((buf (or buf
                    (get-buffer
                     (consult--read (mapcar #'buffer-name (consult--buffer-query
                                                           :sort 'visibility))
                                    :category 'buffer
                                    :require-match
                                    ...
                                    :sort nil))))
           (tab (tab-bar-get-buffer-tab buf (selected-frame))))
      (if tab
          (progn
  ...
  (defun my/display-buffer (buf)
    "Display BUF via display-buffer, with a control with prefix."
    (let* ((prefix (prefix-numeric-value current-prefix-arg)))
      (cond
       ((= prefix 4)
        (switch-to-buffer buf))
       ((= prefix 16)
        (my/switch-to-buffer-other-tab buf))
       ((= prefix 64)
  ...
  (defun my/switch-to-buffer (&optional buf)
    "Switch to an existing tab if the buffer already exists or to a new tab."
    (interactive)
    (let* ((prefix (prefix-numeric-value current-prefix-arg))
           (buf (or buf
                    (get-buffer
                     (consult--read (mapcar #'buffer-name (consult--buffer-query
                                    ...
                                    :history 'consult--buffer-history
                                    :sort nil)))))
      (my/display-buffer buf)))

  (defun my/find-file (filename &optional wildcards)
    "Visit file FILENAME using display-buffer.
    ...
    (let ((value (find-file-noselect filename nil nil wildcards)))
      (if (listp value)
          (progn
            ...
            (my/display-buffer (car value))
            (mapc #'switch-to-buffer (cdr value))
            value)
        (my/display-buffer value))))

  (defun my/tab-bar-kill-buffer-and-tab ()
    "Kill current buffer and its tab."
    (interactive)
    (when (kill-buffer)
      (if (> (length (tab-bar-tabs (selected-frame))) 1)
          (tab-close))))

  (defun my/tab-bar-kill-buffer-and-window-or-tab ()
    "Kill current buffer and its associated window or tab if there is a single window in the tab."
    (interactive)
    (if (> (count-windows) 1)
        (kill-buffer-and-window)
      (my/tab-bar-kill-buffer-and-tab)))

  (defun my/tab-bar-close-duplicate-tabs (&optional buf)
    "Close duplicate tabs."
    (interactive)
    (let* ((buf (or buf (current-buffer)))
           (tabs (tab-bar-get-buffer-tab buf nil t t)))
      (dolist (tab tabs)
...
(use-package consult
  ...
  (defun my/consult-buffer (name items)
    (interactive)
    (cond
     ((null items)
      (message "no buffers specified"))
     ((eq (length items) 1)
      (message "switching to %s" (car items))
      (my/switch-to-buffer (car items)))
     (t
      (consult-buffer `(( :name ,name
                          :hidden nil
                          :narrow ?s
                          :category buffer
                          :face consult-buffer
  ...
  (defun my/consult-mode-buffer (&optional mode)
    (interactive)
    (let ((mode (or mode major-mode)))
      (my/consult-buffer (format "%s buffers" (symbol-name mode))
  ...
  (defun my/consult-recent-file ()
    ...
    (let* ((filename
      ...
      (if (listp value)
          (mapcar 'my/display-buffer (nreverse value))
        (let ((win (my/display-buffer value)))
          (when win
            (select-window win))))))
...
Disabled projectile for now as I've not been using it much.
Adding some config for project.el instead. 
...
#+begin_src emacs-lisp
  (use-package project
    ...
    :bind (:map project-prefix-map
                ("d" . #'my/project-dirvish)
                ("f" . #'my/project-find-file)
                ("e" . #'my/project-eshell)
                ("v" . #'my/project-vterm)
                ("g" . #'my/project-ghostel))
    :custom
    (project-switch-commands '((my/project-dirvish "dirvish")
                               (my/project-find-file "Find File")
                               (my/project-eshell "eshell")
                               (my/project-vterm "vterm")
                               (my/project-ghostel "ghostel")))
    :config
    ...
    (defun my/project-vterm (&optional arg)
      (interactive "p")
      (let ((default-directory (project-root (project-current t)))
            (buf-name (project-prefixed-buffer-name "vterm")))
        (vterm buf-name)))
...
(use-package jit-spell
  ...
  :hook (((org-mode markdown-mode) . jit-spell-mode)
         (text-mode . (lambda () (when (member major-mode '(text-mode)) (jit-spell-mode)))))
  :config
...
** Spacing between latin and non-latin words

#+begin_src emacs-lisp
(use-package pangu-spacing
  :hook ((text-mode org-mode markdown-mode) . pangu-spacing-mode)
  :custom
...
#+begin_src emacs-lisp
(use-package dirvish
  :bind (("C-c t" . dirvish-side)
         :map dirvish-mode-map
         ("i" . dirvish-toggle-subtree)
         ("N" . dirvish-narrow))
  :config
...
Workaround for missing non-ASCII text when pasting to GTK based application.

#+begin_src emacs-lisp
(defun my/wl-copy-region ()
  (interactive)
  (shell-command-on-region (mark) (point) "wl-copy > /dev/null 2>&1"))
#+end_src
...
(use-package treemacs
  :init
  :bind (("C-c T" . treemacs-select-directory))
  :hook ((treemacs-mode . (lambda () (setq-local truncate-lines t))))
  :custom
  (treemacs-default-visit-action 'treemacs-visit-node-vertical-split))
...
(defvar my/max-windows-per-frame 2)
(defvar my/split-window-horizontally-min-width 100)
(defun my/set-split-window-threshold (width)
  (interactive "nWidth: ")
  (setq my/split-window-horizontally-min-width width))
...
return the same window."
  (let* ((num-windows (count-windows))
         ...
         (win (get-buffer-window buf))
         ;; Override split-width/height-threshold to forcibly
         ;; split the preferred direction.
         (split-width-threshold
          (if (>= (frame-width) min-frame-width) 1 nil))
         (split-height-threshold
          (if (< (frame-width) min-frame-width) 1 nil)))
    (cond
     (win win)
     ((not (buffer-file-name buf))  ;; maybe transient buffer
      (display-buffer-at-bottom buf alist))
     ((< num-windows my/max-windows-per-frame)
      (if (>= (frame-width) min-frame-width)
          (progn
            (split-window-right)
...
#+begin_src emacs-lisp
(defun my/shellish-buffer-p (buf &optional filter-mode)
  "Return if BUF is a shell-ish buffer. if FILTER-MODE is nil, buf is compared with major-mode."
  (let ((mode (with-current-buffer buf major-mode)))
    (and (or (string-match-p (format "\\*%s\\*<[0-9]>"
                                 (my/shellish-name (or filter-mode major-mode)))
                         (buffer-name buf))
             ;; temporarily disable the buffername-matching part
             t)
         (if filter-mode
             (eq filter-mode mode)
           (eq major-mode mode)))))
...
(defvar my/display-shellish-buffer-list #'my/consult-shellish-buffers
...
(defun my/find-last-shellish-buffer (buflist shellish-mode)
  "Return most recently used shell-ish buffer containing SHELL-STR in BUFLIST."
  (when buflist
    (if (my/shellish-buffer-p (car buflist) shellish-mode)
        (car buflist) (my/find-last-shellish-buffer (cdr buflist) shellish-mode))))

(defvar my/shellish-use-tab-p nil)
(defvar my/shellish-window-height .5)
(defvar my/shellish-use-tab-per-buffer-alist nil)

(defun my/shellish-set-window-height (&optional height)
  (interactive "nHeight: ")
  (setq my/shellish-window-height height))

(defun my/shellish-get-use-tab-for-buffer (buf)
  (alist-get buf my/shellish-use-tab-per-buffer-alist my/shellish-use-tab-p nil 'string=))

(defun my/shellish-set-use-tab-for-buffer (buf val)
  (setf (alist-get buf my/shellish-use-tab-per-buffer-alist my/shellish-use-tab-p nil 'string=) val))

(defun my/shellish-name (shellish-mode)
  (string-remove-suffix "-mode"
                        (symbol-name shellish-mode)))

(defun my/shellish-tab-name (shellish-mode)
  (format "*%s tab*" (my/shellish-name shellish-mode)))

(defvar my/shellish-max-index 9)
(defun my/shellish-get-new-index (shellish-mode)
  "Return the least vacant index (1-based) for given SHELLISH-MODE. Return nil if none found."
  (let ((n 1)
        (buf-name (my/shellish-name shellish-mode)))
    (while (and (<= n my/shellish-max-index)
                (get-buffer (format "*%s*<%d>" buf-name n)))
      (setq n (1+ n)))
    (if (<= n my/shellish-max-index) n nil)))

(defun my/shellish-buffer-name (shellish-mode n)
  (format "*%s*<%d>" (my/shellish-name shellish-mode) n))

(defun my/shellish-get-keymap-symbol (shellish-mode)
  (intern (format "my/shellish-%s-keymap"
                  (my/shellish-name shellish-mode))))

(defun my/shellish-get-switch-func-symbol (shellish-mode n)
  (intern (format "my/shellish-%s-switch-to-%d"
                  (my/shellish-name shellish-mode) n)))

(defun my/shellish-create-switch-to-function (shellish-mode n)
  (fset (my/shellish-get-switch-func-symbol shellish-mode n)
        `(lambda (arg)
           (interactive "p")
           (my/shellish-toggle-buffer arg (quote ,shellish-mode) ,n))))

(defun my/shellish-create-prefix-command (shellish-mode)
  (let ((keymap-symbol (my/shellish-get-keymap-symbol shellish-mode)))
    (define-prefix-command keymap-symbol)
    (dotimes (n 10)
      (my/shellish-create-switch-to-function shellish-mode n)
      (keymap-set keymap-symbol (format "%d" n)
                  (my/shellish-get-switch-func-symbol shellish-mode n)))
    (keymap-set keymap-symbol "s"
                `(lambda (arg)
                   (interactive "p")
                   (my/shellish-toggle-buffer arg (quote ,shellish-mode))))
    (keymap-set keymap-symbol "t"
                `(lambda ()
                   (interactive)
                   (my/shellish-toggle-buffer 4 (quote ,shellish-mode))))
    keymap-symbol))

(defun my/get-display-buffer-action-for-shellish (shellish-mode use-tab-p)
  "Return display-buffer's ACTION parameter applied to shellish buffers."
  (let ((cur-buf (current-buffer))
        (tab-name (my/shellish-tab-name shellish-mode)))
    (cond
     ((and (my/shellish-buffer-p cur-buf shellish-mode)
           (eq (my/shellish-get-use-tab-for-buffer (buffer-name cur-buf)) use-tab-p))
...
(defun my/shellish-popup-buffer (buf)
  "Popup shellish buffer BUF at a bottom with omni-display setting considered."
  (let ((tab (tab-bar-get-buffer-tab buf (selected-frame)))
        (omni-display (my/shellish-get-omni-display-for-buffer
                        (buffer-name buf))))
    (if (or omni-display (not tab))
...
(defun my/shellish-create-buffer (shellish-mode n)
  (let* ((buf-name (my/shellish-buffer-name shellish-mode n)))
    (pcase shellish-mode
      ('vterm-mode (vterm buf-name))
      ('shell-mode (shell buf-name))
      ('eshell-mode (eshell n))
      ('term-mode (multi-term buf-name))
      ('eat-mode (eat nil n)))))

(defvar my/shellish-omni-display-alist nil)

(defun my/shellish-get-omni-display-for-buffer (buf-name)
  (alist-get buf-name my/shellish-omni-display-alist nil nil 'string=))

(defun my/shellish-toggle-omni-display-for-buffer (buf-name)
  (setf (alist-get buf-name my/shellish-omni-display-alist nil nil 'string=)
        (not (my/shellish-get-omni-display-for-buffer buf-name))))

(defun my/shellish-toggle-omni-display ()
  (interactive)
  (message "shellish omni display %s"
           (if (my/shellish-toggle-omni-display-for-buffer (buffer-name))
               "enabled" "disabled")))
(define-key my/wm-map "O" #'my/shellish-toggle-omni-display)

(defun my/shellish-toggle-buffer (arg shellish-mode &optional buf-index)
  (interactive "p")
  (let* ((use-tab-flipped (= arg 4))
         (select-buffer-p (= arg 16))
         (b (if buf-index (get-buffer (my/shellish-buffer-name shellish-mode buf-index))
              (my/find-last-shellish-buffer (buffer-list) shellish-mode)))
         (new-buf-index (or buf-index
                            (my/shellish-get-new-index shellish-mode)))
         (buf-name (if b (buffer-name b)
                     (my/shellish-buffer-name shellish-mode new-buf-index)))
         (use-tab-p (my/shellish-get-use-tab-for-buffer buf-name))
         (new-use-tab-p (xor use-tab-p use-tab-flipped))
         (create-new-buf-p (= arg 64))
         (display-buffer-overriding-action
          (my/get-display-buffer-action-for-shellish shellish-mode new-use-tab-p)))
    (cond
     ((or (not b) create-new-buf-p)
      (my/shellish-create-buffer shellish-mode new-buf-index))
     (select-buffer-p
      (when b
        (select-window (display-buffer b)))
      (call-interactively my/display-shellish-buffer-list))
     (b
      (let ((tab (tab-bar-get-buffer-tab b (selected-frame)))
            (win (get-buffer-window b))
            (buf-active-p (eq b (current-buffer))))
        ;; Close shellish-buffer if it's current buffer or tab/window conf flipped.
        (when (or buf-active-p use-tab-flipped)
          (if use-tab-p
              (if tab (tab-bar-close-tab-by-name (alist-get 'name tab)))
            (if win
                (condition-case err
                    (delete-window win)
                  (error
                   (switch-to-prev-buffer))))))
        ;; Show shellish buffer unless it was current buffer.
        (unless (and buf-active-p (not use-tab-flipped))
          (if new-use-tab-p
              (my/switch-to-buffer-other-tab b)
            (my/shellish-popup-buffer b))))))
    ;; update the use-tab-p flag for the next call.
    (my/shellish-set-use-tab-for-buffer buf-name new-use-tab-p)))
#+end_src
...
#+begin_src emacs-lisp
(use-package shell
  :bind (("C-c S" . my/get-shell)
         ("C-x ~" . dirs))
  :config
  (defun my/get-shell (arg)
    (interactive "p")
    (my/shellish-toggle-buffer arg 'shell-mode))
...
(use-package eshell
  ...
  (defun my/get-eshell (arg)
    (interactive "p")
    (my/shellish-toggle-buffer arg 'eshell-mode)))
...
(use-package vterm
  ...
  ;; Create shellish commands.
  (my/shellish-create-prefix-command 'vterm-mode)

  ;; Set up *vterm*<0> as omni display buffer.
  (let ((buf-name (my/shellish-buffer-name 'vterm-mode 0)))
      (my/shellish-toggle-omni-display-for-buffer buf-name)
      (my/shellish-set-use-tab-for-buffer buf-name nil))

  (dotimes (n 10)
    (global-set-key (kbd (format "C-c C-%d" n))
                    (my/shellish-get-switch-func-symbol 'vterm-mode n)))

  (defun my/newvterm ()
    "Create a new vterm with specified buffer name."
    (interactive)
    (vterm (format "*vterm*<%d>" (my/shellish-get-new-index 'vterm-mode))))

  (defun my/get-vterm (arg)
    (interactive "p")
    (my/shellish-toggle-buffer arg 'vterm-mode)
    )
...
(use-package multi-term
  ...
  (defun my/get-term (arg)
    (interactive "p")
    (my/shellish-toggle-buffer arg 'term-mode))
...
#+begin_src emacs-lisp
(use-package eat
  :bind (("C-c E" . my/get-eat)
         :map eat-mode-map
         ("C-c C-k" . eat-semi-char-mode))
  :hook ((eat-mode . corfu-mode))  ;; enable corfu completion
  ...
  (defun my/get-eat (arg)
    (interactive "p")
    (my/shellish-toggle-buffer arg 'eat-mode)))
#+end_src
...
#+begin_src emacs-lisp
(use-package ghostel
  :straight (:type git :host github :repo "dakra/ghostel" :files ("lisp/*.el" "etc"))
  :bind (("C-c g" . my/ghostel))
  :custom
  (ghostel-keymap-exceptions
   '("C-c" "C-x" "C-u" "C-h" "C-]" "C-g" "C-z" "M-x" "M-o" "M-:" "C-\\"))
  :config
  (defun my/ghostel ()
    (interactive)
    (let ((prefix (prefix-numeric-value current-prefix-arg)))
      (cond
       ((= prefix 4)
        (my/consult-mode-buffer 'ghostel-mode))
       (t
        (ghostel))))))
#+end_src
...
(use-package lsp-mode
  ...
  (when (file-exists-p "~/bin/emacs-lsp-booster")
...
(use-package puni
  ...
  :bind (("C-d" . delete-char)  ;; don't override C-d
         ...
         ("C-c {" . puni-wrap-curly)
         ("C-c <" . puni-wrap-angle)
         ("C-c ~" . my/puni-wrap-tilde)
         ("C-c `" . my/puni-wrap-backtick)
...
(use-package apheleia
  :bind ("C-c i f" . apheleia-format-buffer))
#+end_src
...
(use-package copilot
  ...
  :bind ( :map copilot-completion-map
          ...
          ("C-c ." . copilot-accept-completion-by-sentence)
          ("C-c ," . copilot-accept-completion-to-char)
...
(use-package agent-shell
  ...
  :bind ( :map my/ai-map
          ("A" . my/agent-shell-map)
          :map my/agent-shell-map
          ("a" . agent-shell-anthropic-start-claude-code)
          ("c" . agent-shell-openai-start-codex)
...
(use-package claude-code
  ...
  ;; (setq claude-code-notification-function #'my/claude-code-notify)

  ;; Display buffer configuration.
  (defun my/claude-code-display-buffer (buf)
    "Display claude code buffer."
    (interactive)
    (select-window (display-buffer-at-bottom buf '((window-height . 0.4)))))
  (setq claude-code-display-window-fn #'my/claude-code-display-buffer)
...
(use-package claude-code-ide
  ...
  :bind ( :map my/ai-map
          ("c" . claude-code-ide-menu)) ; Set your favorite keybinding
  :custom
  ...
  (defun my/claude-code-ide-tab-name (buf alist)
    "Return tab-name for the given claude-code-ide buffer."
    (buffer-name buf))
  (add-to-list 'display-buffer-alist
               '("\\`\\*claude-code\\[.+\\]\\*"
                 ...
                 ;; (window-height . 0.4)))
                 display-buffer-in-tab
                 (tab-name . my/claude-code-ide-tab-name)
                 (window-height . 1.0)
                 (dedicated . t)))
  )
#+end_src
...
(use-package gh-copilot-chat
  ...
  :commands (gh-copilot-chat-transient gh-copilot-chat-display)
  :bind ( :map my/ai-map
          ("C" . gh-copilot-chat-transient))
  :custom
  (gh-copilot-chat-backend 'curl)
  (gh-copilot-chat-frontend 'org)
  (gh-copilot-chat-commit-model "gpt-5-mini")
  )
#+end_src
...
Disabled as emacs got stuck while it was readong a large json file.
...
(use-package pyvenv
  :hook ((after-init . my/pyvenv-setup)
         (python-ts-mode . my/pyvenv-activate))
  :config
  (defun my/pyvenv-activate ()
    ...
    (let ((current-dir default-directory))
      (while (and current-dir
                  (not (file-directory-p (expand-file-name ".venv" current-dir))))
        (setq current-dir (file-name-parent-directory current-dir)))
      (when current-dir
        (let ((venv-dir (expand-file-name ".venv" current-dir)))
          (message "Activating venv with %s" venv-dir)
...
TBH this is not maintained these days.
...
(use-package org
  ...
  (org-agenda-sorting-strategy
   '((agenda habit-down time-up priority-down category-keep)
     (todo urgency-down priority-down todo-state-up category-keep)
  ...
  ;; WORKING status should be used exclusively for time recording
  ;; and should only be active while actively recording time.
  (org-todo-keywords
   '((sequence "TODO(t)" "NEXT(n!)" "ACTIVE(A!)" "ASSIGNED(a!)" "INPROGRESS(i!)" "WORKING(w!/!)"
               "ONHOLD(h@/!)" "BLOCKED(b@/!)" "|" "DONE(d!)" "CANCELLED(c@)")
     (sequence "SHOPPING(s)" "LATER(l)" "|" "DONE(d!)" "CANCELLED(c@)")))
  (org-priority-faces
   '((?A :foreground "black" :background "OrangeRed")
     (?B :foreground "black" :background "ForestGreen")
     (?C :foreground "black" :background "RoyalBlue3")))
  (org-log-done 'time)
  ...
  (org-agenda-custom-commands
   '(("d" "Today"
      ((agenda ""
               ...
               `((:name "Overdue"
                 ...
                 (:name "Due Today"
                        :and ( :todo ("TODO" "NEXT")
                               :deadline today
                               :not (:children todo))
                        :face (:foreground "LightYellow" :weight bold))
                 ...
                 (:name "Due Within a Week"
                        :and ( :todo ("TODO" "NEXT")
                               ...
                               :not (:children todo))
                        :face (:inherit org-upcoming-deadline))
                 (:name "Scheduled Within a Week"
                        :and ( :todo ("TODO" "NEXT")
                               :scheduled (before ,(format-time-string "%Y-%m-%d" (org-read-date nil t "+1w")))
                               :not (:habit t)
                               :not (:children todo))
                        :face (:inherit org-upcoming-deadline))
                 (:discard (:anything t))))
              (org-agenda-sorting-strategy
               '(urgency-down priority-down todo-state-up category-keep))
       ...
       (todo ""
             ((org-super-agenda-groups
               `((:name "Working Now" :and (:todo "WORKING" :not (:children todo))
                        :face (:foreground "Yellow" :weight bold))
                 (:name "In Progress" :and (:todo "INPROGRESS" :not (:children todo))
                        :face (:foreground "#ffff00"))
                 (:name "Assigned" :and (:todo ("ASSIGNED" "NEXT") :not (:children todo))
                        :face (:foreground "Gold"))
                 (:name "Blocked" :and (:todo "BLOCKED" :not (:children todo))
                        :face (:foreground "#8888bb"))
                 (:name "On Hold" :and (:todo "ONHOLD" :not (:children todo))
                        :face (:foreground "#8888bb"))
                 (:discard (:anything t))))
              (org-agenda-sorting-strategy
               '(urgency-down priority-down todo-state-up category-keep))
       ...
       (tags-todo "+PROJECT=\"personal\"-TODO=\"SHOPPING\"-TODO=\"LATER\"-STYLE=\"habit\"-recurring"
                  ((org-super-agenda-groups
                    '((:todo "ASSIGNED")
                      (:todo "NEXT")
                      (:todo "TODO")
                   ...
                   (org-agenda-overriding-header "Non-recurring Personal Tasks Awaiting Processing")
                   (org-agenda-prefix-format
                    '((tags . " %-2i %-18(my/org-get-todo-date-str)   ")))))
       (tags-todo "-PROJECT=\"personal\"-TODO=\"SHOPPING\"-TODO=\"LATER\"-STYLE=\"habit\""
                  ((org-use-property-inheritance '("project"))
                   (org-super-agenda-groups
                    '((:name "ASSIGNED" :and (:todo "ASSIGNED"  :not (:children todo)))
                      (:name "NEXT" :and (:todo "NEXT"  :not (:children todo)))
                      (:name "ONHOLD" :and (:todo "ONHOLD"  :not (:children todo)))
                      (:discard (:anything t))))
      ...
      ((tags-todo "+PROJECT=\"personal\"+STYLE=\"habit\""
                  (
                   ...
                   (org-agenda-prefix-format
                    '((tags . " %-2i %-18(my/org-get-todo-date-str) ")))))))))
  (org-stuck-projects
   '("+PROJECT/-DONE-SOMEDAY" ("TODO" "BLOCKED" "ONHOLD")))
  ...
  (custom-set-faces
   ...
   '(org-scheduled-previously ((t :foreground "#ff3300" :inherit default)))
   ...
   '(org-headline-done ((t :barkground "gray20" :strike-through t))))
  ...
  (defun my/org-agenda-custom-view ()
    ...
    (when (file-directory-p my/org-roam-project-root)
      (add-to-list 'recentf-exclude #'my/org-agenda-recentf-exclude)
      (org-agenda nil "d")
      (delete 'my/org-agenda-recentf-exclude recentf-exclude)))

  (require 'seq)
  (defun my/org-agenda-recentf-exclude (file)
    (when (boundp 'org-agenda-files)
      (not (null (seq-some (lambda (pat)
                             (string-search (expand-file-name pat)
  ...
  (defun my/org-agenda-startup ()
    (when
        (and (not
              (string-match-p "--\\(eval\\|batch\\|daemon\\)"
                            (string-join command-line-args " ")))
            (string= (buffer-name) "*scratch*"))
      (my/org-agenda-custom-view)))

  (add-hook 'emacs-startup-hook #'my/org-agenda-startup)
  ...
  ;; Helper function to schedule the last day of the month.
  ;; usage: (my/end-of-month '(2 28 2017))
  ;; borrowed from https://emacs.stackexchange.com/questions/31683/schedule-org-task-for-last-day-of-every-month
  (defun my/end-of-month (date &optional days-back)
    ...
    (let* ((day (calendar-extract-day date))
      ...
      (if days-back
          (>= (+ day days-back) last-day-of-month)
...
(use-package org-clock
  ...
  :custom-face
  (org-agenda-clocking ((t :foreground "Yellow" :weight bold :box (:line-width (1 . 1) :color "Yellow"))))
  :custom
  (org-clock-persist t)
  :config
...
(use-package org-roam
  ...
  :bind (("C-c n f" . org-roam-node-find)
         ...
         ("C-c o p c" . my/org-roam-project-capture-custom)
 ...
 property and additional properties to the new file."
  ...
  (defun my/org-roam-project-create-new-file (prj plist)
    ...
    (let* ((prj-dir (my/org-roam-project-dir prj))
           ...
           (file (my/org-roam-project-file prj basename))
           (title (or (plist-get plist :title) (format "[%s] %s" prj basename)))
  ...
  (defun my/org-roam-project-basenames (prj)
    (mapcar (lambda (i)
              (string-remove-suffix ".org" i))
            (seq-remove
             (lambda (i) (member i '("personal.org" "tasks.org")))
             (directory-files
              (file-name-concat my/org-roam-project-root prj)
              nil "\\`[^.]"))))

  (defun my/org-roam-project-get-custom-params ()
    (interactive)
    (let* ((prj (my/org-roam-project-read))
           (basename
            (completing-read "basename: "
                             (my/org-roam-project-basenames prj)
                             nil 'confirm)))
      (list prj basename)))

  (defun my/org-roam-project-capture-custom (prj basename)
    "Capture new journal for the selected project."
    (interactive (my/org-roam-project-get-custom-params))
    (my/org-roam-project-capture prj `( :basename ,basename
                                        :target file
                                        :headline "* %?")))
...
(defun my/org-gtasks-setup ()
  (interactive)
  (let* ((el "~/lib/elisp/org-gtasks/org-gtasks.el")
         (authinfo (car (auth-source-search :host "org-gtasks")))
         (login (plist-get authinfo :user))
         (client (plist-get authinfo :client))
         (secret (auth-source-pick-first-password :host "org-gtasks")))
    (when (file-exists-p el)
      (load el)
      (org-gtasks-register-account :name "default"
                                   :directory "~/.emacs.d/gtasks/"
                                   :login login
                                   :client-id client
                                   :client-secret secret))))

(when (file-exists-p "~/org/gtasks.org")
  (add-to-list 'org-agenda-files "~/org/gtasks.org" 'append))
#+end_src
...
(use-package calfw
  ...
  (setq calfw-item-separator-color-face nil)

  (custom-set-faces
   '(calfw-title-face ((t (:foreground "#f0dfaf" :weight bold :height 2.0 :inherit variable-pitch))))
   '(calfw-header-face ((t (:foreground "#cccccc" :background "#666666" :weight bold))))
   '(calfw-sunday-face ((t :foreground "#cc9393" :background "#662222" :weight bold)))
   '(calfw-saturday-face ((t :foreground "#8cd0d3" :background "RoyalBlue" :weight bold)))
   '(calfw-holiday-face ((t :foreground "#cccccc" :background "#662222" :weight bold)))
   '(calfw-grid-face ((t :foreground "DarkGrey")))
   '(calfw-default-content-face ((t :foreground "#bfebbf")))
   '(calfw-periods-face ((t :foreground "cyan")))
   '(calfw-day-title-face ((t :background "#333333")))
   '(calfw-default-day-face ((t :weight bold :inherit calfw-day-title-face)))
   '(calfw-annotation-face ((t :foreground "PalmGreen" :inherit calfw-day-title-face)))
   '(calfw-disable-face ((t :foreground "DarkGray" :inherit calfw-day-title-face)))
   '(calfw-today-title-face ((t :foreground "white" :weight bold)))
   '(calfw-today-face ((t :background: "white" :weight bold)))
   '(calfw-select-face ((t :background "#666666")))
   '(calfw-toolbar-face ((t :foreground "Steelblue4")))
   '(calfw-toolbar-button-off-face ((t :foreground "gray" :weight bold)))
   '(calfw-toolbar-button-on-face ((t :foreground "gray" :weight bold))))
...
(use-package calfw-org
  :after (calfw org)
  :straight nil
  :bind (("C-c o C" . my/calfw-org-open-calendar))
  :commands (my/calfw-org-open-calendar)
  :init
  ...
  (defun my/calfw-org-open-calendar ()
    (interactive)
    (calfw-org-open-calendar (org-agenda-files nil 'ifmode)
                             "org-agenda"
                             my/default-face-foreground)))
#+end_src
...
(defvar my/llm-provider-alist
  '((gemini :auth-source "gemini-api-key" :envvar "GEMINI_API_KEY")
    ...
    (zai :auth-source "zai-api-key" :envvar "ZAI_API_KEY")
    (nvidia :auth-source "nvidia-api-key")
    (opencode :auth-source "opencode-api-key" :envvar "OPENCODE_API_KEY")))
...
(use-package ellama
  ...
  (defmacro my/make-llm-opencode (&rest args)
    `(make-llm-openai-compatible
      :url "https://opencode.ai/zen/go/v1/"
      :key (my/get-opencode-api-key)
      ,@args))

  (defmacro my/make-llm-openrouter (&rest args)
    `(make-llm-openai-compatible
      :url "https://openrouter.ai/api/v1/"
      :key (my/get-openrouter-api-key)
      ,@args))

  (defmacro my/make-llm-zai (&rest args)
    `(make-llm-openai-compatible
      :url "https://api.z.ai/api/coding/paas/v4/"
      :key (my/get-zai-api-key)
      ,@args))
  ...
  (setopt ellama-provider (my/make-llm-groq
  ...
  (setopt ellama-providers
          '(("gemini-2.5-flash-image-preview" . (my/make-llm-gemini
            ...
            ("gemini-flash-lite-latest" . (my/make-llm-gemini
                                                :chat-model "gemini-flash-lite-latest"))
            ("gemini-flash-latest" . (my/make-llm-gemini
                                           :chat-model "gemini-flash-latest"))
            ("gemini-2.5-pro" . (my/make-llm-gemini
                                         :chat-model "gemini-2.5-pro"))
            ("gemini-3-flash-preview" . (my/make-llm-gemini
                                         :chat-model "gemini-3-flash-preview"))
            ("gemini-3-pro-preview" . (my/make-llm-gemini
                                         :chat-model "gemini-3-pro-preview"))
            ("gemini-3-pro-image-preview" . (my/make-llm-gemini
                                         :chat-model "gemini-3-pro-image-preview"))
            ("groq-gpt-oss-120b" . (my/make-llm-groq
                                    :chat-model "openai/gpt-oss-120b"))
            ("opencode-mimo-v2.5" . (my/make-llm-opencode
                                     :chat-model "mimo-v2.5"))
            ("opencode-kimi-k2.6" . (my/make-llm-opencode
                                     :chat-model "kimi-k2.6"))
            ("opencode-deepseek-v4-pro" . (my/make-llm-opencode
                                     :chat-model "deepseek-v4-pro"))
            ("opencode-deepseek-v4-flash" . (my/make-llm-opencode
                                     :chat-model "deepseek-v4-flash"))
            ("openrouter-mimo-v2-flash" . (my/make-llm-openrouter
                                     :chat-model "xiaomi/mimo-v2-flash"))
            ("openrouter-glm-4.5-air" . (my/make-llm-sambanova
                                     :chat-model "z-ai/glm-4.5-air:free"))
            ("openrouter-deepseek-r1t2-chimera" . (my/make-llm-sambanova
                                     :chat-model "tngtech/deepseek-r1t2-chimera:free"))
            ("sambanova-gpt-oss-120b" . (my/make-llm-sambanova
                                     :chat-model "gpt-oss-120b"))
            ("sambanova-gwen3-235b" . (my/make-llm-sambanova
                                     :chat-model "Qwen3-235B"))
            ("zai-glm-4.7" . (my/make-llm-zai
                                     :chat-model "GLM-4.7"))
            ("zai-glm-5.1" . (my/make-llm-zai
                                     :chat-model "GLM-5.1"))
            ("zai-glm-5.2" . (my/make-llm-zai
                                     :chat-model "GLM-5.2"))))

  ;; Disable the nickname code; switching models mid-session won't reflect the update.
...
(use-package gptel
  ...
  (gptel-make-openai "OpenCode-Minimax"
    :host "opencode.ai"
...
o              openai/gpt-oss-120b:free
  ...
  ;; Set default backend.
  (setq gptel-backend (my/gptel-get-backend "Z.ai"))
...
(use-package mcp
  ...
  :custom (mcp-hub-servers
           `(;; ("filesystem" . (:command "npx"
             ...
             ;;                  :roots (,(file-name-concat (getenv "HOME") "prj"))))
...
(use-package elfeed
  ...
  :commands (my/elfeed-update-async)
  :config
  ;; Add time to the search result.
  (defun elfeed-search-format-date (date)
    (format-time-string "%m-%d %H:%M" (seconds-to-time date)))

  ;;
  (defun my/elfeed-format-column-advice (orig-fn string width &optional align)
    "pad a space when needed to support titles with multi-bytes better"
    (let ((title (apply orig-fn (list string width align))))
      (if (= (1+ (string-width title)) width)
          (concat title " ")
        title)))

  (advice-add #'elfeed-format-column :around #'my/elfeed-format-column-advice)

  ;; Patched version - adjusted title-width for my date format.
  (defun elfeed-search-print-entry--default (entry)
    ...
    (let* ((date (elfeed-search-format-date (elfeed-entry-date entry)))
           ...
           (tags-str (mapconcat
                      ...
                      tags ","))
           (title-width (- (window-width) 11 elfeed-search-trailing-width))
  ...
  (advice-add #'elfeed-search-show-entry :before
              (lambda (entry &optional preview) (delete-other-windows)))
  ...
  ;; Update in-memory DB with the db in the filesystem assuming it is newer.
  (defun my/elfeed-update-inmemory-db (&optional days-back)
    "Load elfeed-db from memory and merge read state."
    (interactive "nDays back: ")
    (let ((entries-read ())
          (cutoff-time (- (float-time) (* (or days-back 30) 86400))))
      ;; Save entries' read state.
      (with-elfeed-db-visit (entry feed)
        (when (and (>= (elfeed-entry-date entry) cutoff-time)
                   (not (elfeed-tagged-p 'unread entry)))
          (add-to-list 'entries-read entry)))
      ;; Load DB from the filesystem.
      (elfeed-db-load)
      ;; Update entries with current read state.
      (dolist (entry entries-read)
        (let ((entry-in-db (gethash (elfeed-entry-id entry) elfeed-db-entries)))
          (when entry-in-db
            (elfeed-untag entry-in-db 'unread)))))
    (elfeed-db-set-update-time)
    (elfeed-search-update))

  (advice-add #'elfeed-search-quit-window :before
              (lambda () (my/elfeed-update-inmemory-db 60)))

  (defun my/elfeed-db-save ()
    (interactive)
    (when (yes-or-no-p "Really save DB? ")
      (elfeed-db-save)))

  ;; For async update with emacsclient --eval.
  (defun my/elfeed-update-async ()
    "Update elfeed feeds in a headless way."
    (interactive)
    (let ((proc
           (async-start
            `(lambda ()
               ,(async-inject-variables "\\`\\(load-path\\|rmh-elfeed-org-files\\)\\'")
               (require 'elfeed)
               (require 'elfeed-org)
               (elfeed-org)
               (elfeed-db-load)
               (rmh-elfeed-org-process-advice)
               (let ((last-db-update (elfeed-db-last-update))
                     (updated nil))
                 (elfeed-update)
                 (while (> (elfeed-queue-count-total) 0)
                   (sleep-for 1))
                 (setq updated (> (elfeed-db-last-update) last-db-update))
                 (when updated
                   (elfeed-db-save))
                 updated))
            (lambda (updated)
              (when updated
                ;; update DB
                (my/elfeed-update-inmemory-db))
              (message "[%s] elfeed-update done.%s"
                       (format-time-string "%Y-%m-%d %H:%M:%S")
                       (if updated
                           (format " elfeed-db updated (%s)."
                                   (with-current-buffer (elfeed-search-buffer)
                                     (elfeed-search--count-unread)))
                         ""))))))))

  (defun my/elfeed (&optional days-back)
    "Launch elfeed after loading on-disk db and merge unread tags."
    (interactive "P")
    (my/elfeed-update-inmemory-db days-back)
    (elfeed)))
...
(use-package consult-omni
  ...
  :bind ( :map my/search-map
          ...
          ("n" . my/consult-omni-notes-search)
  ...
  ;; Start async ripgrep process as soon as input is given.
  (defun my/consult-omni-notes-search (&optional input prompt no-callback &rest args)
    "Query consult-omni-notes-search with the minimum input threshold."
    (interactive "P")
    (let ((consult-omni-async-min-input 1))
      (consult-omni-notes-search input prompt no-callback args)))
  )
#+end_src
...
(use-package telega
  ...
  (telega-use-docker t)
  :config
  (setq telega-docker--user-id "0:0"))
#+end_src
