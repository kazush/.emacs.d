...
(use-package consult
  ...
  :bind (;; C-c bindings (mode-specific-map)
         ("C-c r" . my/consult-recent-file-new-window)
        ...
        (while buflist
          (let* ((b (car buflist))
            ...
            (if (or (eq b (current-buffer))
                    ...
                    (seq-contains '(exwm-mode shell-mode eshell-mode term-mode vterm-mode eat-mode)
...
(use-package eat
  ...
  :config
  (defun my/neweat ()
    "Create a new eat with specified buffer name."
    (interactive)
    (if (my/find-last-shellish-buffer (buffer-list (selected-frame)) "eat")
        (vterm (format "*eat<%s>*"
                       (read-string "Shell buffer name: ")))
      (eat)))
  ...
  (setopt ellama-providers
          '(("gemini-flash" . (my/make-llm-gemini
            ...
            ("gemini-flash-latest" . (my/make-llm-gemini
                               :chat-model "gemini-1.5-flash-latest"))
            ("gemini-pro-latest" . (my/make-llm-gemini
