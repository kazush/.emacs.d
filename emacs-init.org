...
(defun my/shellish-toggle-buffer (arg shellish-mode &optional buf-index)
  (interactive "p")
  (let* ((select-buffer-p (= arg 4))
         (use-tab-flipped (= arg 16))
    ...
    (cond
     ...
     ;; ((and (string= current-tab-name tab-name)
     ;;       (eq b (current-buffer)))
     ...
     (b ;; switch to existing shellish buffer
      ;; close dedicated tab if exists and not used.
      ;; (when (and tab-exists-p
      ;;            (not new-use-tab-p))
      ;;   (tab-bar-close-tab-by-name tab-name))
      ;; move focus to the shellish buffer
      (select-window (display-buffer b))))
